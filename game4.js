function fire() {
  ...
  const offs = p.scythes === 1 ? [0] : p.scythes === 2 ? [0, 0.18 * (p.face >= 0 ? 1 : -1)] : [-0.26, 0, 0.26];
  for (const o of offs) {
    const a = base + o;
...
function updateCamera(force) {
  ...
  const tx = clampn(G.p.x - W / 2, 0, Math.max(0, WORLD_W - W));
  const ty = clampn(G.p.y - H / 2, 0, Math.max(0, WORLD_H - H));
  if (force) { G.cam.x = tx; G.cam.y = ty; }
  else {
    G.cam.x += (tx - G.cam.x) * 0.18;
    G.cam.y += (ty - G.cam.y) * 0.18;
  }
  G.camX = Math.round(G.cam.x);
  G.camY = Math.round(G.cam.y);
}
function draw() {
  ...
  updateCamera();
  const sh = G.shake;
  const shakeX = sh > 0 ? Math.round((Math.random() - 0.5) * sh * 2) : 0;
  const shakeY = sh > 0 ? Math.round((Math.random() - 0.5) * sh * 2) : 0;
  ctx.save();
  ctx.translate(-G.camX + shakeX, -G.camY + shakeY);
  ctx.drawImage(mapCanvas, 0, 0);
  if (G.mode) { ctx.fillStyle = `rgba(255,${120 + Math.floor(Math.sin(G.frame * 0.3) * 40)},0,${0.1 + Math.sin(G.frame * 0.4) * 0.04})`; ctx.fillRect(G.camX, G.camY, W, H); }
  const p = G.p;
  ...
  if (T && T.phase === 'warn') {
    if (Math.floor(T.t / 5) % 2 === 0) {
      ...
      if (T.axis === 'h') { ctx.moveTo(0, T.pos); ctx.lineTo(WORLD_W, T.pos); } else { ctx.moveTo(T.pos, 0); ctx.lineTo(T.pos, WORLD_H); }
      ctx.stroke(); ctx.setLineDash([]);
    }
    const wx = T.axis === 'h' ? (T.dir > 0 ? G.camX + 6 : G.camX + W - 30) : T.pos - 12, wy = T.axis === 'h' ? T.pos - 22 : (T.dir > 0 ? G.camY + 24 : G.camY + H - 40);
  ...
  for (const e of G.enemies) {
    ...
    drawHuman(SPR[e.type], e.x, e.y, e.face, e.anim, true, e.flash, 1);
