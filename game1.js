'use strict';
const W = 270, H = 480, TS = 16, MW = 36, MH = 50;
...
function buildMap(stage) {
  ...
  const riverX = 6 + Math.floor(rnd() * (MW - 12));
  for (let y = 0; y < MH; y++) { tiles[y][riverX] = 3; if (riverX + 1 < MW) tiles[y][riverX + 1] = 3; }
  const pathY = 8 + Math.floor(rnd() * (MH - 16));
  for (let x = 0; x < MW; x++) tiles[pathY][x] = tiles[pathY][x] === 3 ? 4 : 1;
  const pathX = 4 + Math.floor(rnd() * (MW - 8));
  for (let y = 0; y < MH; y++) if (tiles[y][pathX] === 0) tiles[y][pathX] = 1;
  const b2 = clampn(pathY + 10, 1, MH - 2);
  tiles[b2][riverX] = 4; if (riverX + 1 < MW) tiles[b2][riverX + 1] = 4;
  for (let i = 0; i < 12; i++) {
    const px = 1 + Math.floor(rnd() * (MW - 4)), py = 1 + Math.floor(rnd() * (MH - 4));
  ...
  tryPlace('house', 2, 2, 2 + (stage % 3));
  tryPlace('tree', 1, 1, 22);
  tryPlace('rock', 1, 1, 12);
  for (const o of objs) {
  ...
  mapCanvas = document.createElement('canvas'); mapCanvas.width = WORLD_W; mapCanvas.height = WORLD_H;
  ...
  for (let y = 0; y < MH; y++) for (let x = 0; x < MW; x++) {
    ...
    if (t === 0) {
      ...
      if (rnd() < 0.18) { g.fillStyle = '#6cc058'; g.fillRect(px + Math.floor(rnd() * 12), py + Math.floor(rnd() * 12), 1, 3); }
    } else if (t === 1) {
