...
function dropItem(x, y) {
  ...
  G.items.push({ type, x: Math.max(8, Math.min(WORLD_W - 8, x)), y: Math.max(8, Math.min(WORLD_H - 8, y)), life: 600 });
}
...
function spawnEnemy(type, sx, sy) {
  ...
  if (x === undefined) {
    for (let i = 0; i < 8; i++) {
      ...
      if (s === 0) { x = -12; y = Math.random() * WORLD_H; } else if (s === 1) { x = WORLD_W + 12; y = Math.random() * WORLD_H; }
      else if (s === 2) { x = Math.random() * WORLD_W; y = -12; } else { x = Math.random() * WORLD_W; y = WORLD_H + 12; }
...
function spawnBoss() {
  ...
  const bx = clampn(G.p.x, 24, WORLD_W - 24);
  const by = clampn(G.p.y - 160, -40, WORLD_H - 120);
  const ty = clampn(G.p.y - 120, 48, WORLD_H - 120);
  G.boss = { x: bx, y: by, targetY: ty, hp, maxHp: hp, state: 'enter', t: 0, cd: 110, face: 1, flash: 0, vx: 0, vy: 0, enraged: false, pat: 0 };
...
function spawnTsuji() {
  ...
  const pos = axis === 'h' ? Math.max(14, Math.min(WORLD_H - 14, p.y)) : Math.max(14, Math.min(WORLD_W - 14, p.x));
...
function updateGame() {
  ...
  for (const e of G.enemies) {
    ...
    if (e.type === 'ninja') {
      ...
      if (e.cd <= 0 && dist < 170 && e.x > 0 && e.x < WORLD_W && e.y > 0 && e.y < WORLD_H) { G.ebullets.push({ x: e.x, y: e.y, vx: ux * 2.6, vy: uy * 2.6, life: 120, kind: 'shuriken', rot: 0 }); e.cd = 90 + Math.random() * 50; }
    ...
    if (e.ghost > 0 || e.x < 4 || e.x > WORLD_W - 4 || e.y < 4 || e.y > WORLD_H - 4) { e.x += ddx; e.y += ddy; }
  ...
  if (T) {
    if (T.phase === 'warn') { if (--T.t <= 0) T.phase = 'dash'; }
      ...
      if (T.x < -30 || T.x > WORLD_W + 30 || T.y < -30 || T.y > WORLD_H + 30) G.tsuji = null;
    }
  }
  const b = G.boss;
  if (b && b.state !== 'dead') {
    ...
    if (b.state === 'enter') { b.y += 1; if (b.y >= b.targetY) { b.state = 'move'; b.t = 0; } }
      ...
      b.t--;
      if (b.t === 20) shoot(b.enraged ? 9 : 7);
      if (b.enraged && b.t === 4) shoot(9);
      if (b.t <= 0) { b.state = 'move'; b.cd = b.enraged ? 60 : 110; }
      ...
      b.x += b.vx; b.y += b.vy;
      if (b.x < 16 || b.x > WORLD_W - 16) b.vx *= -1; if (b.y < 30 || b.y > WORLD_H - 16) b.vy *= -1;
      if (G.frame % 2 === 0) burst(b.x, b.y + 12, '#c9a978', 2, 1.5);
      if (dist < 20) hurtPlayer(2);
      if (--b.t <= 0) { b.state = 'move'; b.cd = b.enraged ? 60 : 110; }
    }
    if (b.state !== 'dash' && b.state !== 'enter' && dist < 18) hurtPlayer(1);
    b.x = Math.max(16, Math.min(WORLD_W - 16, b.x)); b.y = Math.max(-40, Math.min(WORLD_H - 16, b.y));
  }
  for (const s of G.scythes) {
    ...
    for (const e of G.enemies) {
      ...
      if (Math.hypot(e.x - s.x, e.y - s.y) < s.r) { s.hits.add(e); damageEnemy(e, s.dmg, s.vx, s.vy); if (!s.pierce) { dead = true; break; } }
    }
    ...
    if (dead || s.life <= 0 || s.x < -10 || s.x > WORLD_W + 10 || s.y < -10 || s.y > WORLD_H + 10) s.dead = true;
  }
  G.scythes = G.scythes.filter(s => !s.dead);
  for (const q of G.ebullets) {
    q.x += q.vx; q.y += q.vy; q.life--; if (q.kind === 'shuriken') q.rot += 0.3;
    if (Math.hypot(q.x - p.x, q.y - p.y) < 8) { hurtPlayer(1); q.life = 0; }
    if (p.shield > 0 && Math.hypot(q.x - p.x, q.y - p.y) < 14) { q.life = 0; burst(q.x, q.y, '#ffe040', 3, 1); }
  }
  G.ebullets = G.ebullets.filter(q => q.life > 0 && q.x > -10 && q.x < WORLD_W + 10 && q.y > -10 && q.y < WORLD_H + 10);
  for (const it of G.items) {
    ...
    if (Math.hypot(it.x - p.x, it.y - p.y) < 11) {
      ...
      if (it.type === 'waraji') p.sandal = 600;
      else if (it.type === 'onigiri') p.hp = Math.min(p.maxHp, p.hp + 2);
      else if (it.type === 'kama') { if (p.scythes < 3) p.scythes++; else G.score += 500; }
      else if (it.type === 'akakama') { if (p.power < 3) p.power++; else G.score += 500; }
      else { p.inv = Math.max(p.inv, 240); p.shield = 240; }
    }
  }
